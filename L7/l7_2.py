...
# Функция создания Г-образной маски
def gamma_mask(size):
    mask = np.zeros((size, size), dtype=np.float32)
    # Вертикальная часть (левая колонка)
    mask[:, 0] = 1.0
    # Горизонтальная часть (нижняя строка)
    mask[-1, :] = 1.0
    # Нормировка
...
# ==============================================
print("\n=== Задание 3 ===")
# Вариант 1: Маска двоения (вертикальное)
double_psf = double_mask(15)
restored_double = restore_image(input_image, double_psf, method='wiener')

# Вариант 2: Г-образная маска
gamma_psf = gamma_mask(15)
restored_gamma = restore_image(input_image, gamma_psf, method='wiener')
...
plt.title('Восстановлено (двоение)')
plt.axis('off')

plt.subplot(2, 2, 3)
plt.imshow(cv2.cvtColor(restored_gamma, cv2.COLOR_BGR2RGB))
plt.title('Восстановлено (Г-образная маска)')
plt.axis('off')
...
cv2.imwrite(f'{output_dir}/3_restored_double.jpg', (restored_double * 255).astype(np.uint8))
cv2.imwrite(f'{output_dir}/3_restored_gamma.jpg', (restored_gamma * 255).astype(np.uint8))
